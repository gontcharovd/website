...
var searchLimit = -1; // global variable to store search limit
      ...
      if (xhr.status === 200) {
        ...
        if (data) {
          ...
          if (params.fuseOpts) {
            ...
            // Store the limit value for use in search
            searchLimit = params.fuseOpts.limit ? params.fuseOpts.limit : -1;
          }
...
sInput.onkeyup = function (e) {
  ...
  // in the search box
  if (fuse) {
    const searchOptions = searchLimit > 0 ? { limit: searchLimit } : {};
    const results = fuse.search(this.value.trim(), searchOptions); // the actual query being run using fuse.js
    if (results.length !== 0) {
